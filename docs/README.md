...
## 🎯 Custom Gallery Reordering Feature

This app includes a custom feature that allows reordering products in the gallery based on scoring data from an external API. This feature is particularly useful for category pages where specific product positioning is required.

### How it works

1. **API Integration**: The gallery component automatically fetches ordering data from `/api/dataentities/OG/search` endpoint
2. **Scoring System**: Products are reordered based on a scoring system where lower scores indicate higher priority
3. **Fallback Behavior**: Products without scoring data are placed at the end of the list
4. **Performance**: The reordering is memoized and only recalculates when products or ordering data changes
...
- **Automatic Detection**: The feature automatically detects when a category query is present
- **Smart Order Detection**: Automatically detects if there's already an existing order in runtimeQuery and omits custom ordering
- **Smart API Calls**: API is only called when category/collection changes and no existing order is present
- **Error Handling**: Graceful fallback to original product order if API fails
- **Caching**: Efficient memoization prevents unnecessary re-calculations and API calls
- **Performance Optimized**: Uses category tracking and order detection to avoid redundant API requests
- **TypeScript Support**: Full type safety with proper interfaces

### Example Usage

The feature works automatically when the gallery component is used in category pages. No additional configuration is required - it will:

1. Extract the category from the search query
2. Fetch ordering data from the API
3. Reorder products based on scoring
4. Display the reordered gallery

Products with scoring 1 will appear first, followed by scoring 2, 3, etc. Products without scoring data will appear at the end in their original order.

### Smart Order Detection

The component intelligently detects when there's already an existing order in the URL parameters (`runtimeQuery.order`). In such cases:

- **Existing Order Present**: The custom ordering logic is completely bypassed
- **No Existing Order**: The custom ordering from the API is applied
- **Performance Benefit**: Prevents unnecessary API calls when order is already specified

This ensures compatibility with VTEX's built-in ordering system while providing custom ordering when needed.
